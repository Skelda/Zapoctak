...
def drawInsertion(
    ...
    canvas_insertion = tk.Canvas(window, width=width, height=height)
    canvas_insertion.place(x=0, y=height + 26)
    max_data = max(data)
    tk.Label(
    ...
    for i, j in enumerate(data):
        if i == index:
            drawRectangle(canvas_insertion, len(data), i, "green", max_data, j)
        elif i == index + 1:
            drawRectangle(canvas_insertion, len(data), i, "red", max_data, j)
        else:
            drawRectangle(canvas_insertion, len(data), i, "blue", max_data, j)
